...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'manish' AND `password` = 'manpreet'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('manish', 'manpreet')")
        conn.commit()
...
def LoginForm():
    ...
    lbl_text = Label(TopLoginForm, text="Administrator Login", bg="#009ACD", font=('arial', 18), width=600)
    ...
    btn_login = Button(MidLoginForm, text="Login", font=('arial', 18), bg="#009ACD", width=30, command=Login)
...
def Home():
    ...
    Title.pack(pady=100)
    lbl_display = Label(Title, text="WelCome...", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
...
def AddNewForm():
    ...
    lbl_text = Label(TopAddNew, text="Add New Product", font=('arial', 18), bg="#009ACD", width=600)
    lbl_text.pack(fill=X)
...
def ViewForm():
    ...
    lbl_text = Label(TopViewForm, text="View Products", font=('arial', 18), bg="#009ACD", width=600)
    lbl_text.pack(fill=X)
...
def Delete():
    if not tree.selection():
        ...
        messagebox.showinfo('Inventory System', 'Please select Item than hit delete!')
        #print("ERROR")
    else:
